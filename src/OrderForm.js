...
const OrderForm = () => {
    ...
    const handleEditProduct = (index) => {
        ...
        setEditingProduct({ ...productToEdit, index });
        
        // Populate the form based on the product type
        switch (productToEdit.type) {
            case 'Motorcycle':
                setMotorcycle(productToEdit);
                break;
            case 'Car':
                setCar(productToEdit);  // Corrected
                break; 
            case 'Truck':
                setTruck(productToEdit);  // Corrected
                break; 
            case 'Trailer':
                setTrailer(productToEdit);  // Corrected
                break;
            // ...
        }
    ...
    const handleSaveChanges = () => {
        ...
        switch (editingProduct.type) {
            case 'Motorcycle':
                updatedProduct = { ...motorcycle, type: 'Motorcycle' };
                break;
            case 'Car':
                updatedProduct = { ...car, type: 'Car' };
                break;
            case 'Truck':
                updatedProduct = { ...truck, type: 'Truck' };
                break;
            case 'Trailer':
                updatedProduct = { ...trailer, type: 'Trailer' };
                break;
            break;
        }
    const updatedProducts = [...products];
    updatedProducts[editingProduct.index] = updatedProduct;
    ...
    const resetFormFields = () => {
        setMotorcycle({ brand: '', model: '', engineCapacity: '', colour: '' });
        setCar({ brand: '', model: '', engineCapacity: '', colour: '', numberOfBeds: '', category: '' });
        setTruck({ brand: '', model: '', engineCapacity: '', colour: '', numberOfBeds:'' });
        setTrailer({ brand: '', model: '', loadCapacity: '', numberOfAxles: '' });
    ...
    return (
        ...
        <div className="form-container">
            <h1>{editingProduct ? 'Editing Product' : 'Order Form'}</h1>
            <form onSubmit={handleSubmit} className="container mt-5">
            <div className="mb-3">
                <label htmlFor="productType" className="form-label">Choose a Product Type:</label>
                <select id="productType" value={productType} onChange={handleProductTypeChange} className="form-select" disabled={editingProduct != null}>
                ...
                <div>
                    ...
                    {editingProduct ? (
                        <div className="button-container">
                            <button type="button" className="btn btn-secondary mb-3 btn-flat" onClick={handleCancelEdit}>
                                Cancel
                            </button>
                            <button type="button" className="btn btn-success mb-3 btn-flat" onClick={handleSaveChanges}>
                                Save Changes
                            </button>
                        </div>
                    ) : (
                        <button type="button" className="btn btn-primary mb-3 btn-flat" onClick={() => addProduct('Motorcycle')}>
                            Add Motorcycle
                        </button>
                    )}
                </div>
            ...
            {productType === 'Car' && (
                <div>
                    ...
                    {editingProduct ? (
                        ...
                        <button type="button" className="btn btn-primary mb-3 btn-flat" onClick={() => addProduct('Car')}>
                            Add Car
                        </button>
                    )}
                </div>
            ...
            {productType === 'Truck' && (
                <div>
                    ...
                    {editingProduct ? (
                        ...
                        <button type="button" className="btn btn-primary mb-3 btn-flat" onClick={() => addProduct('Truck')}>
                            Add Truck
                        </button>
                    )}
                </div>
            ...
            {productType === 'Trailer' && (
                <div>
                    ...
                    {editingProduct ? (
                        ...
                        <button type="button" className="btn btn-primary mb-3 btn-flat" onClick={() => addProduct('Trailer')}>
                            Add Trailer
                        </button>
                    )}
                </div>
            ...
            <button type="button" onClick={togglePreview} className="home-button logout-button">Preview Order</button> 
                    ...
                    {products.map((product, index) => (
                        <div key={index} className="preview-item">
                            ...
                            {product.type === 'Motorcycle' && (
                                <>
                                    <div><strong>Engine Capacity:</strong> {product.engineCapacity}</div>
                            ...
                            {product.type === 'Car' && (
                                <>
                                    <div><strong>Engine Capacity:</strong> {product.engineCapacity}</div>
                                    <div><strong>Colour:</strong> {product.colour}</div>
                                    <div><strong>Number of Doors:</strong> {product.numberOfDoors}</div>
                                    <div><strong>Car Category:</strong> {product.category}</div>
                                </>
                            )}
                            {product.type === 'Truck' && (
                                <>
                                    <div><strong>Engine Capacity:</strong> {product.engineCapacity}</div>
                                    <div><strong>Colour:</strong> {product.colour}</div>
                                    <div><strong>Number of Beds:</strong> {product.numberOfBeds}</div>
                                </>
                            )}
                            {product.type === 'Trailer' && (
                                <>
                                    <div><strong>Load Capacity:</strong> {product.loadCapacity}</div>
                                    <div><strong>Number of Axles:</strong> {product.numberOfAxles}</div>
                                </>
                            )}
                            {/* Add edit button if required */}
                            <button type="button" onClick={() => handleEditProduct(index)} className="btn btn-secondary btn-sm">Edit</button>
                            <button type="button" onClick={() => handleDeleteProduct(index)} className="btn btn-danger btn-sm">Delete</button>
      
                        </div>
                    ))}
                    <div>
                    <button type="submit" className="home-button logout-button" disabled={isSubmitDisabled}>Submit Order</button>
                    </div>           
                </div>
            )}
            </form>
